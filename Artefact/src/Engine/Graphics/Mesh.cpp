...
Mesh::Mesh(ID3D11Device* pD3DDevice, const std::vector<Shader::VertexPosColor> pVertex, const std::vector<WORD> pIndicies, const DirectX::XMFLOAT3 pPosition, const DirectX::XMFLOAT3 pRotation, const DirectX::XMFLOAT3 pScale) {
	...
	if (FAILED(hr)) { FatalAppExit(0, "Failed to create Vertex Buffer Descripter"); }
...
void Mesh::Render(ID3D11Device* d3dDevice, ID3D11DeviceContext* d3dDeviceContext, 
	...
	const UINT vertexStride = sizeof(Shader::VertexPosColor);
	const UINT offset = 0;
...
Mesh* Mesh::LoadFromFile(ID3D11Device* device, std::string path, const DirectX::XMFLOAT3 pPosition, const DirectX::XMFLOAT3 pRotation, const DirectX::XMFLOAT3 pScale) {
	std::vector<Shader::VertexPosColor> vertices;
