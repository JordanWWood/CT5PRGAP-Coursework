...
namespace Artefact {
    ...
    void Camera::Update(Shader* p_Shaders, Window* p_Window) const {
        p_Shaders->Update(Shader::CB_Frame, DirectX::XMMatrixLookAtLH(XMLoadFloat3(&m_EyePosition), XMLoadFloat3(&m_FocusPoint), XMLoadFloat3(&m_UpDirection)));
    }
}
