...
{
public:
	Mesh(ID3D11Device* pD3DDevice, const std::vector<Shader::VertexPosColor> pVertex, const std::vector<WORD> pIndicies, const DirectX::XMFLOAT3 pPosition, const DirectX::XMFLOAT3 pRotation, const DirectX::XMFLOAT3 pScale);
	~Mesh();

	void Render(ID3D11Device* m_d3dDevice, ID3D11DeviceContext* m_d3dDeviceContext,
		Shader* m_Shaders, ID3D11RenderTargetView* pD3DRenderTargetView, ID3D11DepthStencilState* pD3DDepthStencilState,
